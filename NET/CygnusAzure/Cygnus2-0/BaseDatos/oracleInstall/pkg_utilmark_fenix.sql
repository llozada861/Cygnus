...
AS
    ...
    (
        ...
        onuRefCredma    OUT tyRefCursor,
        onuRefUsuari    OUT tyRefCursor,
        onuRefObjsbl    OUT tyRefCursor,
        onuRefUsersO    OUT tyRefCursor,
        onuErrorCode    OUT NUMBER,
        osbErrorMessage OUT VARCHAR2
    );
...
AS
    ...
    IS
        sbValoresExcluir VARCHAR2(4000); --pkg_epm_boparametr.fsbget('EXC_MENSAJE_MARK');
        nuCodigo         NUMBER; --mensaje.menscodi%type := 0;
    BEGIN
        pInicializaError(onuErrorCode,osbErrorMessage);
    ...
    BEGIN
        ...
        OPEN onuRefCursor FOR
            ...
            AND    owner IN (SELECT usuario FROM ll_credmark);
    ...
    BEGIN
        ...
        OPEN onuRefCredma FOR
            SELECT 'INSERT INTO ll_credmark (usuario,codigo) values ('''||usuario||''','''||codigo||''');' valor
            FROM ll_credmark;
                    
        OPEN onuRefUsuari FOR
            SELECT 'INSERT INTO ll_usuarios (usuario,rol,email) values ('''||usuario||''','''||rol||''','''||email||''');' valor
            FROM ll_usuarios
            UNION
            SELECT 'INSERT INTO ll_version (version,fecha_ini,fecha_fin) values ('''||version||''','''||fecha_ini||''','''||fecha_fin||''');' valor
            FROM ll_version;
                        
        OPEN onuRefObjsbl FOR
            SELECT 'INSERT INTO ll_objetosbl (codigo,objeto,orden,usuario,bloqueado,fecha_bloqueo,fecha_registro,owner,fecha_est_lib) values ('||codigo||','''||objeto||''','''||orden||''','''||usuario||''','''||bloqueado||''','''||fecha_bloqueo||''','''||fecha_registro||''','''||owner||''','''||fecha_est_lib||''');' valor
            FROM ll_objetosbl
            WHERE bloqueado = 'Y';  
            
        /*OPEN onuRefUsersO FOR
            select 'insert into sa_user (USER_ID, MASK, DB_USER_NAME, OWNER_ID, USER_TYPE_ID, LAST_SUCC_LOGON_DATE, CREATION_DATE, LAST_UPDATE_DATE, LAST_UPDATE_USER, DEFAULT_TABLESPACE, TEMPORARY_TABLESPACE, PATH_REPORT, COMPANY_ID, HAS_MULTISESSION, GUID, COMMON_NAME, TENANT_ID) '||
            ...
            and mask in (select usuario from ll_usuarios)
            UNION
            select 'INSERT INTO ge_person (PERSON_ID, PERSONAL_TYPE, NAME_, NUMBER_ID, PHONE_NUMBER, E_MAIL, BEEPER, GEOGRAP_LOCATION_ID, IDENT_TYPE_ID, USER_ID, EMPLOYEE_COMPANY_ID, COMMENT_, FAX_NUMBER, ATEL_PHONE_NUMBER, EXTERNAL_CODE, ADDRESS_ID, BANK_ID, BRANCH_ID, POPUP_NOTI_ENABLE, COMPANY_ID) '||
    ...
    BEGIN
        ...
        INSERT INTO ll_logapli (codigo,fecha_registro,objeto,maquina,usuario,tipo,accion,objetos_inv,esquema)
        VALUES
        (
            seq_ll_logapli.nextval,
            sysdate,
            isbObjeto,
            isbMaquina,
            UPPER(isbUsuario),
    ...
    BEGIN
        ...
        INSERT INTO ll_credmark (usuario, codigo)
        VALUES
        (
            isbUsuario,
            isbCodigo
    ...
    BEGIN
        ...
        INSERT INTO ll_usuarios (usuario, rol,email)
        VALUES
        (
            isbUsuario,
            isbCodigo,
    ...
    BEGIN
        ...
        OPEN onuRefCursor FOR
            ...
            b.orden,
            b.fecha_est_lib
            FROM all_objects a, ll_objetosbl b
    ...
    IS 
        dtFecha     DATE;
        sbOrden     ll_objetosbl.orden%type;
        sbUsuario   ll_objetosbl.usuario%type;
        dtFechaBlo  ll_objetosbl.fecha_bloqueo%type;
        dtFechaLib  ll_objetosbl.fecha_liberacion%type;
    ...
    BEGIN
       ...
       IF(isbIdAzure = 0)THEN
            onuSeqNeg := seq_ll_negativa.nextval*-1;
            nuIdAzure := onuSeqNeg;
       ELSE
       ...
       IF(nvl(nucodigo,0) = 0)THEN
           ...
           VALUES 
           (nucodigo,isbDescripcion,nuIdAzure,isbEstado,isbUsuario,NULL, (dtFechaFin+7),inuCompletado,SYSDATE);
       ELSE
    ...
    BEGIN
        pInicializaError(onuErrorCode,osbErrorMessage);
        nucodigo := seq_ll_horashoja.nextval;        
                
        INSERT INTO ll_horashoja
    ...
    BEGIN
        ...
        DELETE FROM ll_horashoja
        WHERE codigo =  inuHorasHoja;
        
        COMMIT;
        
        BEGIN
            DELETE FROM ll_requerimiento 
    ...
    BEGIN
        ...
        OPEN cuTotalAzure;
        FETCH cuTotalAzure INTO onuTotalAzure;
        CLOSE cuTotalAzure;
    ...
    BEGIN
        ...
        INSERT INTO ll_usuarios (usuario, rol,email,usuarioAzure)
        VALUES
        (
            isbUsuario,
            NULL,
    ...
    IS
        rcOrigen     ll_horashoja%ROWTYPE;
        rcDestino    ll_horashoja%ROWTYPE;    
    
        CURSOR cuOrigen
        IS
            SELECT hh.*
            FROM ll_horashoja hh,ll_requerimiento rq
